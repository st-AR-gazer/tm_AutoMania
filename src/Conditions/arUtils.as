...
namespace _IO {
    ...
    namespace File {
        ...
        void WriteFile(string _path, const string &in content, bool verbose = false) {
            string path = _path;
            if (verbose) log("Writing to file: " + path, LogLevel::Info, 82, "WriteFile");

            if (path.EndsWith("/") || path.EndsWith("\\")) { log("Invalid file path: " + path, LogLevel::Error, 84, "WriteFile"); return; }
        ...
        void CopySourceFileToNonSource(const string &in originalPath, const string &in storagePath, bool verbose = false) {
            if (verbose) log("Moving the file content", LogLevel::Info, 131, "CopySourceFileToNonSource");
            
            string fileContents = ReadSourceFileToEnd(originalPath);
            WriteFile(storagePath, fileContents);

            if (verbose) log("Finished moving the file", LogLevel::Info, 136, "CopySourceFileToNonSource");
    ...
    void OpenFolder(const string &in path, bool verbose = false) {
        if (IO::FolderExists(path)) {
            ...
            if (verbose) log("Folder does not exist: " + path, LogLevel::Info, 179, "OpenFolder");
        }
    }
}
...
namespace _Game {
    ...
    bool HasPersonalBest(const string &in mapUid, bool verbose = false) {
        ...
        if (verbose) log(mapUid + " | " + pbTime, LogLevel::Debug, 259, "HasPersonalBest");
        return pbTime > 0;
    }
...
namespace _Net {
    ...
    namespace Hidden {
        void CoroDownloadFileToDestination(const string &in userdata) {
            ...
            if (parts.Length != 5) {
                log("Invalid userdata format.", LogLevel::Error, 310, "CoroDownloadFileToDestination");
                return;
            }
            string url = parts[0];
            ...
            if (request.ResponseCode() == 200) {
                ...
                if (!IO::FileExists(tmp_path)) { log("Failed to save file to: " + tmp_path, LogLevel::Error, 357, "CoroDownloadFileToDestination"); return; }
                if (!IO::FileExists(destination)) { log("Failed to move file to: " + destination, LogLevel::Error, 358, "CoroDownloadFileToDestination"); return; }

                IO::Delete(tmp_path);

                if (!IO::FileExists(tmp_path) && IO::FileExists(destination)) {
                    log("File downloaded successfully, saving " + file_name + " to: " + destination, LogLevel::Info, 363, "CoroDownloadFileToDestination");
                ...
                log("Failed to download file. Response code: " + request.ResponseCode(), LogLevel::Info, 377, "CoroDownloadFileToDestination");
            }
        }
    }
}
